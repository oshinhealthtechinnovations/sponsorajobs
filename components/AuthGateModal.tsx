...
export function AuthGateModal() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setSuccessMsg("🎉 Account verified & unlocked! Redirecting...");
        ...
        if (data.requireTrial) {
          // Promo code was wrong
        }
      }
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-lg rounded-3xl bg-slate-950 border border-slate-800 shadow-2xl p-6 sm:p-8 space-y-6 text-white overflow-hidden">
        ...
        {activeTab === "register" && (
          <form onSubmit={handleRegister} className="space-y-3.5">
            ...
            >
              ...
              <span>{loading ? "Verifying Promo Code..." : "Create Account & Unlock"}</span>
            </button>
          </form>
        )}
