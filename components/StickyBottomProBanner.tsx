...
export function StickyBottomProBanner() {
  ...
  const handleOpenProGate = () => {
    window.dispatchEvent(
      new CustomEvent("open-pro-gate", {
        detail: {
          featureName: "All 9,300+ Sponsor Jobs & Direct Application Links",
        },
      })
    );
  };
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 p-2.5 sm:p-3 pb-[calc(0.625rem+env(safe-area-inset-bottom,0px))] bg-white/95 text-slate-900 border-t border-amber-300/80 shadow-2xl backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-2 sm:px-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div>
            <div className="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
              ...
              <span className="text-xs sm:text-sm font-black text-amber-700">
                VIP Pass unlocks 9,300+ verified sponsor jobs + direct apply links.
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          <button
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 min-h-[44px] rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-slate-950 font-black text-xs sm:text-sm transition-all shadow-md active:scale-[0.98] cursor-pointer shrink-0 touch-manipulation"
          >
          ...
          <button
            ...
            className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors cursor-pointer touch-manipulation"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
