            ...
            {gridPosts.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {gridPosts.map((post) => (
                  <article
                    key={post.id}
                    className="group bg-white rounded-2xl border border-slate-200/90 hover:border-brand-500/40 overflow-hidden flex flex-col justify-between transition-all hover:shadow-lg hover:-translate-y-0.5"
                  >
                    {post.featuredImageUrl && (
                      <Link href={`/blog/${post.slug}`} className="block aspect-[16/9] overflow-hidden bg-slate-900">
                        <img
