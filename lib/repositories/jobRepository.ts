...
export class JobRepository {
  ...
  private mapToDTO(
    ...
    return {
      ...
      location: {
        city: typeof job.city === "string" ? job.city : (job.location && typeof job.location === "object" ? (job.location as any).city : null),
        region: job.region || null,
        country: typeof job.country_code === "string" ? job.country_code : (job.location && typeof job.location === "object" ? (job.location as any).country : "GB"),
        formatted: typeof job.location === "string" ? job.location : (job.location && typeof job.location === "object" ? ((job.location as any).formatted || (job.location as any).raw || (job.location as any).city) : `${job.city ? job.city + ", " : ""}${job.country_code || ""}`),
      },
  ...
  async search(params: JobSearchParams): Promise<JobSearchResult> {
    ...
    if (params.company) {
      ...
      const spaceComp = cleanComp.replace(/-/g, " ");
      const hyphenComp = cleanComp.replace(/\s+/g, "-");
      const strippedComp = cleanComp.replace(/[^a-z0-9]/g, "");

      conditions.push(`(
        LOWER(c.name) LIKE ? OR LOWER(c.name) LIKE ? OR
        LOWER(c.normalized_name) LIKE ? OR LOWER(c.normalized_name) LIKE ? OR
        LOWER(c.id) LIKE ? OR LOWER(c.id) LIKE ? OR
        LOWER(j.company_id) LIKE ? OR LOWER(j.company_id) LIKE ? OR
        REPLACE(LOWER(c.name), ' ', '') LIKE ?
      )`);
      const term1 = `%${cleanComp}%`;
      const term2 = `%${spaceComp}%`;
      const term3 = `%${hyphenComp}%`;
      const term4 = `%${strippedComp}%`;
      bindings.push(term1, term2, term1, term2, term1, term3, term1, term3, term4);
    }
  ...
  async getFallbackSuggestions(
    ...
    if (params.country && params.country !== "ALL" && params.country !== "all") {
      const res = await this.search({ country: params.country, limit, sort: "sponsorship", isFallback: true });
      if (res.jobs.length > 0) return res.jobs;
    }
    // 3. Global top-scoring sponsorship jobs
    const res = await this.search({ limit, sort: "sponsorship", isFallback: true });
    return res.jobs;
  }
